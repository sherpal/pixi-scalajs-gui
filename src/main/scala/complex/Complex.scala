...
object Complex {
  ...
  def log(z: Complex): Complex = math.log(!z) + i * z.arg // principal branch of log

  def log(z: Complex, branch: Double): Complex = math.log(!z) + i * (z.arg + (if (z.arg < branch) 2 * Pi else 0))
